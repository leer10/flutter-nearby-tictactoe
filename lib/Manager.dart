...
sub.listen((msg) {
  List<PlayerWithRole_Color> colorList = List.from(PlayerWithRole_Color.values);
  List<PlayerWithRole_Symbol> symbolList = List.from(PlayerWithRole_Symbol.values);
  colorList.shuffle();
  symbolList.shuffle;
  RoleList roleList = RoleList();
  List<SerializablePlayer> listofPlayers = playersBox.toMap().cast<String,Uint8List>().values.map((f) => SerializablePlayer.fromBuffer(f)).where((player) => player.intentPlay == true);
